...
int w=0;
    for (int index = 0; index < ids.length; ++index) {
//if the courses at index equals to the course then print his course and grade
    if (courses[index].equals(course) & h < grades[index]  ) {
h = grades[index];
    w = index;
    System.out.format("[%d]:%s has the highest %d grade in %s\n", ids[w], studentsName[w], grades[w], course  );
         }
     }
 }
...
int w1=0;
         for (int index = 0; index < ids.length; ++index) {
        //if the courses at index equals to the course then print his course and grade
        if ( h1 > grades[index] & courses[index].equals(course)) {
            h1= grades[index];
            w1= index;
        System.out.format("[%d]:%s has the lowest %d grade in %s\n", ids[w1], studentsName[w1], grades[w1], course  );
         }
     }
 }
